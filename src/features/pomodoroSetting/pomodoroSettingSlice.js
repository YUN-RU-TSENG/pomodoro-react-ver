...
export const getAndInitialPomodoroSetting = createAsyncThunk(
    ...
    async (_, { getState }) => {
        try {
            ...
            await setDoc(doc(db, 'pomodoroSetting', userStore.user.uid), initPomodoroSetting)
...
export const updatePomodoroSetting = createAsyncThunk(
    ...
    async (currentSetting, { getState }) => {
        try {
            const { user: userStore } = getState()
            await setDoc(doc(db, 'pomodoroSettings', userStore.user.uid), currentSetting)
...
export const pomodoroSettingSlice = createSlice({
    ...
    extraReducers(builder) {
        builder
            ...
            .addCase(getAndInitialPomodoroSetting.pending, (state, action) => {
                state.ErrorOfPomodoroSettingGet = null
                state.isLoadingPomodoroSettingGet = true
            })
            .addCase(getAndInitialPomodoroSetting.fulfilled, (state, action) => {
        ...
        builder
            ...
            .addCase(updatePomodoroSetting.pending, (state, action) => {
